      ...
      <ul className="space-y-2 text-sm" aria-label="Spending by category">
        {chartData.slice(0, 6).map((d) => (
          <li key={d.name} className="flex items-center gap-2">
            ...
            <span className="w-24 text-right font-medium tabular-nums">{formatCurrency(d.value, 'INR', { maximumFractionDigits: 0 })}</span>
          </li>
        ))}
        {chartData.length > 6 && (
          <li className="text-xs text-slate-400">+{chartData.length - 6} more categories</li>
        )}
      </ul>
    </div>
  )
