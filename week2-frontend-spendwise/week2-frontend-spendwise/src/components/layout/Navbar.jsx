...
/** Avatar + name when signed in, otherwise a "Sign in" link. */
function UserMenu({ compact = false, onNavigate }) {
  ...
  const signOut = () => {
    auth.logout()
    onNavigate?.()
    toast.info('Signed out – you are now in guest mode')
    navigate('/')
  }

  return (
    <div className={`flex items-center gap-2 ${compact ? 'justify-between' : ''}`}>
      <span className="flex items-center gap-2 text-sm" title={auth.user.email}>
        <span
          aria-hidden="true"
          className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-100 text-xs font-bold text-brand-700 dark:bg-brand-900/50 dark:text-brand-300"
        >
          {initials}
        </span>
        <span className={`max-w-[9rem] truncate font-medium ${compact ? '' : 'hidden lg:inline'}`}>
          {auth.user.name}
        </span>
      </span>
      <button
        type="button"
        onClick={signOut}
        className="rounded-lg px-2 py-1.5 text-xs font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white"
      >
        Sign out
      </button>
    </div>
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/85 backdrop-blur dark:border-slate-800 dark:bg-slate-950/85">
      ...
      >
        <ul className="flex flex-col gap-1">
          ...
          <li className="border-t border-slate-200 pt-2 dark:border-slate-800">
            <UserMenu compact onNavigate={closeMenu} />
          </li>
          <li>
            <Link
              to="/dashboard?new=1"
