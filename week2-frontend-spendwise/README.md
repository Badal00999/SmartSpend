...
> **YUVA Internship · Week 2 Task – Front-End Application Development**
...
| `npm test` | Run the automated test suite once (35 tests). |
...
| **DummyJSON** | `GET https://dummyjson.com/users/1?select=firstName,lastName,email,image,company` | Simulated logged-in user on the dashboard. |
| **localStorage** (repository) | `spendwise.transactions.v1`, `spendwise.settings.v1` | Persisting transactions and theme. |
...
**Result: 5 files, 35 tests, all passing.**
...
* Data lives in the browser (`localStorage`); a real REST/GraphQL back-end could be plugged in by replacing `services/storage.js`.
* Single demo user – authentication was out of scope for a front-end-focused week.
...
*Built by a YUVA intern for Week 2 · September 2026*
