...
export default function TransactionDetailPage() {
  ...
  const { transactions, getById, updateTransaction, deleteTransaction, loading } = useTransactions()
  ...
  if (!tx && loading) {
    return (
      <div className="mx-auto max-w-3xl space-y-4 px-4 py-16 sm:px-6" role="status" aria-label="Loading transaction">
  ...
  const handleEdit = async (values) => {
    try {
      ...
      toast.error(err.message || 'Could not update transaction')
    }
  }
