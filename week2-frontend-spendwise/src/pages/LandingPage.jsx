...
const FEATURES = [
  ...
  },
  {
    ...
    text: 'Your data stays in your browser. No sign-up, no servers, no tracking.',
  },
]

const STEPS = [
  { n: '01', title: 'Add a transaction', text: 'Hit "Add", fill in the amount, pick a category and save.' },
  { n: '02', title: 'Explore your dashboard', text: 'Filter, search and sort your history; watch the charts update live.' },
  { n: '03', title: 'Drill into details', text: 'Open any transaction to view, edit or delete it – plus see its share of your spend.' },
]
          ...
          <div>
            ...
            <dl className="mt-10 grid grid-cols-3 gap-4 border-t border-slate-200 pt-6 dark:border-slate-800">
              <div>
                ...
                <dd className="mt-1 text-lg font-bold sm:text-xl">{formatCurrency(totals.balance, 'INR', { maximumFractionDigits: 0 })}</dd>
              </div>
              <div>
            ...
            <div className="card rotate-1 p-5 shadow-xl transition-transform duration-500 hover:rotate-0">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{formatCurrency(totals.balance, 'INR', { maximumFractionDigits: 0 })}</p>
                </div>
              ...
              <div className="mt-5 flex h-28 items-end gap-2">
                {[40, 65, 50, 80, 60, 95, 70].map((h, i) => (
                  <div key={i} className="flex-1 rounded-t-md bg-brand-200 dark:bg-brand-900" style={{ height: `${h}%` }}>
              ...
              <ul className="mt-5 space-y-3">
                {[
                  ...
                  <li key={t} className="flex items-center gap-3 text-sm">
                    <span className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ backgroundColor: `${c}22`, color: c }}>
          ...
          <ol className="mt-12 grid gap-8 md:grid-cols-3">
            {STEPS.map((s) => (
              <li key={s.n} className="relative pl-14">
                <span className="absolute top-0 left-0 flex h-10 w-10 items-center justify-center rounded-full bg-brand-600 text-sm font-bold text-white" aria-hidden="true">
                  {s.n}
                </span>
